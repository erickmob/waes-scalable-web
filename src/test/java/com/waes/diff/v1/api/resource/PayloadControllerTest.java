...
@WebMvcTest(PayloadController.class) class PayloadControllerTest {
  ...
  @Test void addLeftPayload_whenValidData_AddToCollection() throws Exception {
    given(payloadService.save(any(Payload.class))).willReturn(create().message(createdMessage(ID_1)));
    mockMvc.perform(post("/v1/diff/{id}/left", ID_1).contentType(APPLICATION_JSON_VALUE).content(payloadContent(leftPayload1())))
            .andExpect(status().isCreated()).andExpect(jsonPath("$.message", Is.is(createdMessage(ID_1))))
  ...
  @Test void addRightPayload_whenValidData_AddToCollection() throws Exception {
    given(payloadService.save(any(Payload.class))).willReturn(create().message(createdMessage(ID_2)));
    mockMvc.perform(post("/v1/diff/{id}/right", ID_2).contentType(APPLICATION_JSON_VALUE).content(payloadContent(rightPayload1())))
            .andExpect(status().isCreated()).andExpect(jsonPath("$.message", Is.is(createdMessage(ID_2))))
            .andExpect(content().contentType(APPLICATION_JSON_VALUE));
    verify(payloadService).save(any(Payload.class));
  }

  @Test void whenAddLeftPayloadWithInvalidBody_returnNotFound() throws Exception {
    mockMvc.perform(post("/v1/diff/{id}/left", ID_1).content(INVALID_PAYLOAD_CONTENT).contentType(APPLICATION_JSON_VALUE))
  ...
  @Test void whenAddRightPayloadWithInvalidBody_returnNotFound() throws Exception {
    mockMvc.perform(post("/v1/diff/{id}/right", ID_2).content(INVALID_PAYLOAD_CONTENT).contentType(APPLICATION_JSON_VALUE))
            .andExpect(status().isBadRequest()).andExpect(jsonPath("$.content", Is.is("Content should not be null or empty")));
  }
}
